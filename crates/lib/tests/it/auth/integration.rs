...
fn test_tree_default_authentication() {
    let (instance, mut user, tree, default_key) =
        setup_user_and_tree_with_key("test_user", "DEFAULT_KEY");
...
fn test_validation_pipeline_with_concurrent_settings_changes() {
    let (instance, mut user) = crate::helpers::test_instance_with_user("test_user");
...
fn test_prevent_auth_corruption() {
    ...
    let op_valid = tree.new_transaction().expect("Failed to create operation");
