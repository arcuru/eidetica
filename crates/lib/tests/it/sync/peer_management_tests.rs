...
fn test_list_peers() {
    ...
    assert!(pubkeys.contains(&TEST_PEER_PUBKEY.to_string()));
    assert!(pubkeys.contains(&TEST_PEER_PUBKEY_2.to_string()));
}
...
fn test_get_tree_peers() {
    ...
    assert_eq!(peers.len(), 2);
    assert!(peers.contains(&TEST_PEER_PUBKEY.to_string()));
    assert!(peers.contains(&TEST_PEER_PUBKEY_2.to_string()));
