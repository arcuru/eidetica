...
pub async fn create_database_with_settings(
    user: &mut User,
    tree_name: &str,
    version: &str,
) -> Database {
...
pub async fn test_tree_load_workflow(user: &mut User) -> (ID, Database) {
    ...
    // Load tree from ID
    let loaded_tree = user
        .open_database(&root_id)
        .await
...
pub async fn update_tree_metadata(
    tree: &Database,
    name: &str,
    version: &str,
    description: &str,
...
pub async fn test_tree_not_found_error(user: &User, non_existent_name: &str) {
    ...
    if let Err(Error::User(ref e)) = result {
        if let UserError::DatabaseNotFoundByName { ref name } = **e {
            assert_eq!(
                name, non_existent_name,
                "Expected name '{non_existent_name}', got '{name}'"
            );
        } else {
            panic!("Expected DatabaseNotFoundByName error, got: {e:?}");
        }
    } else {
        panic!("Expected DatabaseNotFoundByName error, got an unexpected result");
    }
}
