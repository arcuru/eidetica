...
pub async fn create_database_with_default_key(user: &mut User) -> Database {
    let key_id = user
        .get_default_key()
...
pub async fn create_database_with_settings(user: &mut User, tree_name: &str, version: &str) -> Database {
    let tree = create_database_with_default_key(user).await;

    let op = tree.new_transaction().await.expect("Failed to start operation");
    {
        let settings = op
...
pub async fn create_multiple_named_trees(user: &mut User, names: &[&str]) -> Vec<Database> {
    let mut trees = Vec::new();

    for name in names {
        let tree = create_database_with_settings(user, name, "1.0").await;
        trees.push(tree);
    }
...
pub async fn create_tree_with_data(
    ...
    let op = tree.new_transaction().await.expect("Failed to start operation");
    {
        ...
        for (key, value) in data {
            data_store.set(*key, *value).await.expect("Failed to set data");
        }
    }
...
pub async fn test_tree_load_workflow(user: &mut User) -> (ID, Database) {
    // Create initial tree
    let tree = create_database_with_default_key(user).await;
    ...
    // Load tree from ID
    let loaded_tree = user.open_database(&root_id).await.expect("Failed to load tree");

    (root_id, loaded_tree)
}

/// Create trees for find testing (with various naming scenarios) using User API
pub async fn setup_trees_for_find_testing(user: &mut User) -> Vec<Database> {
    let mut trees = Vec::new();

    // Tree 1: No name
    let tree1 = create_database_with_default_key(user).await;
    trees.push(tree1);

    // Tree 2: Unique name
    let tree2 = create_database_with_settings(user, "UniqueTree", "1.0").await;
    trees.push(tree2);

    // Tree 3: First instance of duplicate name
    let tree3 = create_database_with_settings(user, "DuplicateName", "1.0").await;
    trees.push(tree3);

    // Tree 4: Second instance of duplicate name
    let tree4 = create_database_with_settings(user, "DuplicateName", "2.0").await;
    trees.push(tree4);
...
pub async fn set_tree_settings(tree: &Database, settings_data: &[(&str, &str)]) -> ID {
    let op = tree.new_transaction().await.expect("Failed to start operation");
    {
        ...
        for (key, value) in settings_data {
            settings.set(*key, *value).await.expect("Failed to set setting");
        }
    }
    op.commit().await.expect("Failed to commit settings")
}
...
pub async fn update_tree_metadata(tree: &Database, name: &str, version: &str, description: &str) -> ID {
    let metadata = &[
        ("name", name),
        ("version", version),
        ("description", description),
    ];
    set_tree_settings(tree, metadata).await
}

// ===== DATA OPERATION HELPERS =====

/// Perform basic subtree operations
pub async fn perform_basic_subtree_operations(
    tree: &Database,
    subtree_name: &str,
    operations: &[(&str, &str)],
) -> ID {
...
pub async fn create_user_profile(tree: &Database, user_id: &str, name: &str, email: &str) -> ID {
...
pub async fn create_app_config(tree: &Database, app_name: &str, config_data: &[(&str, &str)]) -> ID {
    let mut all_config = vec![("app_name", app_name)];
    all_config.extend_from_slice(config_data);
    perform_basic_subtree_operations(tree, "app_config", &all_config).await
}

// ===== VERIFICATION HELPERS =====

/// Verify tree has expected settings
pub async fn assert_tree_settings(tree: &Database, expected_settings: &[(&str, &str)]) {
...
pub async fn assert_tree_data(tree: &Database, subtree_name: &str, expected_data: &[(&str, &str)]) {
...
pub async fn assert_tree_name(tree: &Database, expected_name: &str) {
    let actual_name = tree.get_name().await.expect("Failed to get tree name");
...
pub async fn assert_tree_names_in_collection(trees: &[Database], expected_names: &[&str]) {
    let mut tree_names = Vec::new();
    for t in trees {
        if let Ok(name) = t.get_name().await {
...
pub async fn test_tree_not_found_error(user: &User, non_existent_name: &str) {
...
pub async fn test_database_error_conditions(user: &User) {
    // Test that find_database returns proper errors for non-existent databases
    let result = user.find_database("NonExistent").await;
    assert!(
        result.is_err(),
...
pub async fn test_complete_instance_workflow(username: &str) -> (Instance, User, Vec<Database>) {
    let (instance, mut user) = test_instance_with_user(username).await;

    // Create trees with different configurations
    let tree1 = create_database_with_settings(&mut user, "MainTree", "1.0").await;
    ...
    set_tree_settings(&tree2, &[("name", "DataTree"), ("purpose", "user_storage")]).await;
...
pub async fn test_concurrent_tree_operations(user: &mut User) -> Vec<Database> {
    let mut trees = Vec::new();

    // Create multiple trees rapidly
    for i in 0..5 {
        let tree_name = format!("ConcurrentTree{i}");
        let tree = create_database_with_settings(user, &tree_name, "1.0").await;
