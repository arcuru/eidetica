...
fn test_transaction_diamond_pattern() {
    ...
    // Create base entry
    let op_base = ctx.database().new_transaction().unwrap();
    ...
    let op_right = ctx.database().new_transaction_with_tips([base_id]).unwrap();
...
fn test_get_path_from_to_diamond_pattern() {
    ...
    // A is the base
    let op_a = ctx.database().new_transaction().unwrap();
...
fn test_get_path_from_to_diamond_between_lca_and_tip() {
    ...
    // Step 1: Create LCA
    let op_lca = ctx.database().new_transaction().unwrap();
    ...
    let op_d = ctx.database().new_transaction_with_tips([lca_id]).unwrap();
    let store_d = op_d.get_store::<DocStore>("data").unwrap();
...
fn test_correct_lca_and_path_sorting() {
    ...
    let op_root = ctx.database().new_transaction().unwrap();
    ...
    // Branch C (height 1)
    let op_c = ctx.database().new_transaction_with_tips([root_id]).unwrap();
    let store_c = op_c.get_store::<DocStore>("data").unwrap();
    ...
    let op_other = ctx.database().new_transaction_with_tips([c_id]).unwrap();
...
fn test_lca_path_finding_with_helpers() {
    ...
    let lca_scenario = create_lca_test_scenario(ctx.database());
    ...
    assert_lca_path_completeness(ctx.database(), &tips, expected_keys);
}
...
fn test_deterministic_operations_with_helper() {
    ...
    let diamond = create_diamond_pattern(ctx.database());
    let merge_id = create_merge_from_diamond(ctx.database(), &diamond);
    ...
    test_deterministic_operations(ctx.database(), &tips, 10);
}
...
fn test_complex_path_finding_scenario() {
    ...
    let root_id = create_simple_operation(ctx.database(), "data", "root", "value");
