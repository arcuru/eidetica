...
fn test_simple_linear_chain() {
    ...
    let subtree_a = op_a.get_subtree::<DocStore>("data").unwrap();
    ...
    let subtree_b = op_b.get_subtree::<DocStore>("data").unwrap();
    ...
    let subtree_c = op_c.get_subtree::<DocStore>("data").unwrap();
    ...
    let viewer = tree.get_subtree_viewer::<DocStore>("data").unwrap();
...
fn test_caching_consistency() {
    ...
    let viewer1 = tree.get_subtree_viewer::<DocStore>("data").unwrap();
    let state1 = viewer1.get_all().unwrap();

    // Second read - should use cached states
    let viewer2 = tree.get_subtree_viewer::<DocStore>("data").unwrap();
    let state2 = viewer2.get_all().unwrap();

    // Third read - should also use cached states
    let viewer3 = tree.get_subtree_viewer::<DocStore>("data").unwrap();
...
fn test_parent_merge_semantics() {
    ...
    let subtree_base = op_base.get_subtree::<DocStore>("data").unwrap();
    ...
    let subtree_child = op_child.get_subtree::<DocStore>("data").unwrap();
...
fn test_deep_chain_performance() {
    ...
    for i in 1..=CHAIN_LENGTH {
        ...
        let subtree = op.get_subtree::<DocStore>("data").unwrap();
...
fn test_multiple_reads_consistency() {
    ...
    let subtree1 = op1.get_subtree::<DocStore>("data").unwrap();
    subtree1.set("key1", "value1").unwrap();
    subtree1.set("key2", "value2").unwrap();
    op1.commit().unwrap();

    let op2 = tree.new_operation().unwrap();
    let subtree2 = op2.get_subtree::<DocStore>("data").unwrap();
...
fn test_incorrect_parent_merging_would_fail() {
    ...
    let subtree3 = op3.get_subtree::<DocStore>("data").unwrap();
    ...
    let subtree4 = op4.get_subtree::<DocStore>("data").unwrap();
    ...
    for i in 0..5 {
        let viewer_check = tree.get_subtree_viewer::<DocStore>("data").unwrap();
...
fn test_true_diamond_pattern() {
    ...
    let subtree_d = op_d.get_subtree::<DocStore>("data").unwrap();
...
fn test_merge_algorithm_helpers() {
    ...
    let viewer = tree
        .get_subtree_viewer::<DocStore>("data")
...
fn test_merge_performance_with_deep_chains() {
    ...
    let viewer = tree
        .get_subtree_viewer::<DocStore>("performance")
        .expect("Failed to get viewer");
