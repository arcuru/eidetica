...
fn test_settings_metadata_with_complex_operations() {
    ...
    for i in 0..3 {
        let op = tree.new_operation().unwrap();
        let data_store = op.get_subtree::<eidetica::subtree::Dict>("data").unwrap();
...
fn test_metadata_consistency_across_operations() {
    ...
    let auth_store = auth_op
        .get_subtree::<eidetica::subtree::Dict>("auth_data")
        .unwrap();
    ...
    let regular_store = regular_op
        .get_subtree::<eidetica::subtree::Dict>("regular_data")
        .unwrap();
