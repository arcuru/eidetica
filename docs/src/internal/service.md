...
- **Verification status is never asserted over the wire**: the service protocol carries no way for a client to assert a verification status; entries arriving over the wire are stored `Unverified` and earn `Verified` only via a local validation pass. The status model, the pinned-settings validation that makes it staleness-free, the disclosure posture, and the unbuilt authority-*reduction* (revocation) gap are documented in the [Verification Model](../design/verification.md) design doc.
- **Remote-backend detection for `get_tips()` is heuristic**: when a `Database` runs over the remote service backend, the Verified-frontier read path distinguishes the remote case via an error-type check rather than an explicit capability flag. This is a known internal correctness sharp-edge to be replaced with an explicit backend capability signal; behaviour is correct for the current backends but the heuristic is fragile.
