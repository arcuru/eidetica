...
Password-wrapped Stores remain opaque to server maintenance: known wrapper descriptor claims are checked against supported forms before returning `RecordMaintenanceUnavailable`, but `_index` does not expose the encrypted wrapped codec, so the server cannot authenticate which wrapped form is in use. An unlocked `PasswordStore<S>::get_state` on a service connection sends the expected wrapper descriptor through the canonical Read gate, then folds authorized Entry deltas after local decryption; it neither receives a staging token nor publishes records. Wrong passwords fail during `open`, and ciphertext/authentication errors during folding propagate rather than becoming capability refusals. The generic remote `get_store_state::<PasswordStore<S>>` cannot supply a password; callers use the unlocked Store handle. Existing Doc-backed Table continues to use its current record path and `table:v0`.
